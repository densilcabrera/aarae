...
% the following figure is adapted from one by Jonothan Holmes
roomDims = [Lx, Ly, Lz];   % X, Y & Z coords. [m]

src = [xs, ys, zs];      % Source position [m]

rec = [xr,yr,zr];      % receiver position [m]

vertices = [0,0,0;  % normalised vertices of a rectangular prism
    1,0,0;...
    1,1,0;...
    0,1,0;...
    0,0,1;...
...
plotSetup = figure('Name', 'ROOM SETUP');
ax = axes(plotSetup);
view(3)
axis vis3d

 scatter3(ax,rec(1),rec(2),rec(3), markerSize,...
     'MarkerEdgeColor','none',...
     'MarkerFaceColor','r',...
     'DisplayName','rec: P_1')
 hold on
 scatter3(ax,src(1),src(2),src(3), markerSize,...
     'MarkerEdgeColor','none',...
     'MarkerFaceColor','g',...
     'DisplayName','src: P_1')
 legend
...
% plot box
patch(ax,'Vertices',vertices.*roomDims,...
    'Faces',faces,...
    'FaceColor', [255,102,0]./256,...
    'FaceAlpha', 0.05,...
    'EdgeAlpha',1,...
    'DisplayName', 'Room Boundaries')
